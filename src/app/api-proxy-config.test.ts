...
describe('local API proxy configuration', () => {
  it('keeps browser requests relative and proxies them to the configured backend', () => {
    ...
    expect(viteConfig).toContain("env.VITE_API_PROXY_TARGET || 'http://39.97.247.145:8080'");
    expect(viteConfig).toContain('target: apiProxyTarget');
    expect(envExample).toContain('VITE_API_BASE_URL=');
    expect(envExample).toContain('VITE_API_PROXY_TARGET=http://39.97.247.145:8080');
  });
});
